...
const SYSTEM = `Tu es un extracteur de données pour des dossiers contentieux bancaires en Tunisie.
Tu reçois le contenu textuel d'un document juridique (mise en demeure, contrat, jugement, facture, relevé...).
Extrais un ou plusieurs dossiers. Pour les fichiers tabulaires (Excel/CSV), retourne une entrée par ligne.
- Montants en TND (nombre uniquement, sans devise).
- Dates au format ISO YYYY-MM-DD.
- Téléphone : préfixe +216 si numéro tunisien sans indicatif. Email en minuscules.
- Référence : numéro de contrat, dossier ou facture.
    ...
    const resp = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        tools: [{
          type: "function",
          function: {
            name: "extract_contentieux",
            description: "Renvoie la liste des dossiers contentieux extraits.",
            parameters: {
              type: "object",
              properties: {
                dossiers: {
        ...
        tool_choice: { type: "function", function: { name: "extract_contentieux" } },
      }),
    });
